...
### Requirement 2: 音声入力開始・停止と権限
**Objective:** As a ユーザー, I want 音声入力を開始・停止できること, so that 必要なときだけ音声対話を行える
...
3. When 音声入力が開始されたとき, the Voice Chat Web App shall 音声入力の受付を開始する
4. When ユーザーが音声入力の停止を指示したとき, the Voice Chat Web App shall 音声入力の受付と音声由来の応答生成を停止する
5. The Voice Chat Web App shall 音声入力が停止中に音声入力を取得しない

### Requirement 3: ハンズフリー音声入力（VAD含む）
**Objective:** As a ユーザー, I want ハンズフリーで発話できること, so that 操作せずに対話を進められる

#### Acceptance Criteria
1. While 音声入力が有効な間, the Voice Chat Web App shall ユーザーの発話開始と終了を検知する
...
5. The Voice Chat Web App shall 会話セッション内の会話履歴を保持し応答生成に利用する
...
#### Acceptance Criteria
1. The Voice Chat Web App shall 会話状態を明示的に表示する
...
5. The Voice Chat Web App shall 重大なエラー発生時に会話セッションを安全に終了できる

### Requirement 8: 観測性と品質
**Objective:** As a 運用者, I want 品質を把握できること, so that 改善点を特定できる

#### Acceptance Criteria
1. The Voice Chat Web App shall 会話セッション開始・終了などのイベントを記録する
...
2. When 会話セッションが開始されたとき, the Voice Chat Web App shall 双方向チャネルを確立する
3. While 会話セッションが有効な間, the Voice Chat Web App shall 双方向通信の接続状態を維持する
4. If 双方向通信が切断されたとき, the Voice Chat Web App shall ユーザーに通知し再接続を試みる
5. The Voice Chat Web App shall 双方向通信の再接続後に会話セッションの継続または再開を可能にする

### Requirement 10: 音声開始/停止UIと自動再生制約対応
**Objective:** As a ユーザー, I want 明確な音声開始/停止操作ができること, so that ブラウザの制約下でも安全に音声対話を開始できる

#### Acceptance Criteria
1. The Voice Chat Web App shall 音声入力開始のための明確な操作手段を提供する
...
4. While 音声入力が停止中, the Voice Chat Web App shall 音声入力の取得と自動再生を行わない
...
4. The Voice Chat Web App shall 音声入力が停止中にテキスト入力を利用可能にする
5. While 音声入力が開始中, the Voice Chat Web App shall テキスト入力をロックして送信を防止する
